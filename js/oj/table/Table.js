...
OJ.importJs('oj.table.TableColumn');
OJ.importJs('oj.table.TableCell');
OJ.importJs('oj.table.TableRow');

OJ.importCss('oj.table.Table');
...
OJ.compileClass(
	'OjTable',
	oj.components.Table = function(){
		return new oj.list.List(
			arguments, 'OjTable',
			{
				'_template' : 'oj.table.Table',
				...
				'_setDomSource' : function(dom_elm){
					...
					for(i = 0; i < ln; i++){
						...
						if(tag == 'columns'){
							...
							while(ln2-- > 0){
								columns.splice(0, 0, this._createColumn(cols[ln2]));
							}
				...
				'getColumns' : function(){
					...
					while(ln-- > 0){
						columns.splice(0, 0, this.getColumnAt(ln));
					}
